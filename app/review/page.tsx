...
export default function ReviewPage() {
  ...
  const pendingSensitivityRef = useRef<number>(50);
  ...
  useEffect(() => {
    ...
    if (editingSample) {
      ...
      setSensitivity(next);
      pendingSensitivityRef.current = next;
    ...
    if (!sensitivitySliderEnabled) {
      setSensitivity(50);
      pendingSensitivityRef.current = 50;
    }
  ...
  const applySensitivityChange = (nextSensitivityRaw: number) => {
    ...
    pendingSensitivityRef.current = nextSensitivity;
  ...
  const beginSensitivitySliderInteraction = () => {
  ...
  const commitSensitivitySliderInteraction = () => {
    if (!editingSampleId) return;

    scheduleRecheckApply(
      baseThreshold,
      pendingSensitivityRef.current,
      draftMissingCandidateThreshold
    );
  };
      ...
      <div className="px-4 pt-4 pb-3 border-b border-zinc-800 bg-zinc-950 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            disabled={!sensitivitySliderEnabled}
            onPointerDown={beginSensitivitySliderInteraction}
            onTouchStart={beginSensitivitySliderInteraction}
            onMouseDown={beginSensitivitySliderInteraction}
            onChange={(e) => applySensitivityChange(Number(e.target.value))}
            onPointerUp={commitSensitivitySliderInteraction}
            onTouchEnd={commitSensitivitySliderInteraction}
            onMouseUp={commitSensitivitySliderInteraction}
            onBlur={commitSensitivitySliderInteraction}
            ...
            return (
              ...
              >
                <button
                  ...
                  className={`w-full rounded-2xl border px-2 py-2 flex items-center gap-2 min-w-0 transition-all ${
                    selected ? "bg-zinc-800" : "bg-zinc-900"
                  }`}
                  style={{
                    borderColor: outlineColor,
                    borderWidth: selected ? 3 : 1.5,
                    opacity: activeSampleId && !selected ? 0.82 : 1,
                  }}
                >
                  {sample.thumbUrl ? (
                    <div
                ...
                {showDeleteFor === sample.id ? (
                  <div className="absolute -top-2 -right-2 z-50">
                    <button
                      ...
                      className="w-7 h-7 rounded-full bg-rose-500 text-white shadow-lg flex items-center justify-center border border-white/20"
                      aria-label="見本を削除"
                    >
                      ×
                    </button>
