...
const drawPolylineCanvas = (
  ...
  const handleLeaveWithAutoSave = async (path: string) => {
    ...
    if (!autoSaveOn || path !== "/debug/camera") {
      router.push(path);
      return;
    }
    ...
    <main className="min-h-screen bg-black text-white flex flex-col">
      ...
      <div className="px-4 pb-3">
        <div className="grid grid-cols-3 gap-3">
          ...
          {canAdd ? (
            <button
              onClick={() => router.push("/debug/add-sample")}
      ...
      <div className="bg-black px-5 pt-2 pb-6">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => void handleLeaveWithAutoSave("/debug/camera")}
...
function preventContextMenu(e: { preventDefault: () => void }) {
  e.preventDefault();
}
}